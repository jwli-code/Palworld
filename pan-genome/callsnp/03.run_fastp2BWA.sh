...
#cat ./NG_unsuccess.txt|while read id;do
#sra7为sra文件存储位置
ls ${workplace}/sra7 |while read id;do
...
#   [ ! -e ${workplace}/02.FinalBam/${i}.sorted.markdup.bam ] && [ ! -e ${workplace}/03.OriGVCF/${i}_gvcf.gz ];then
   bsub -J ${i} -n $nt -R span[hosts=1] -o ${workplace}/log/${i}.out -e ${workplace}/log/${i}.err -q normal \
   "sh fastp2BWA_v1.sh $i $nt ${workplace} ${workplace}/00.cleandata $ref"
#   fi
done
